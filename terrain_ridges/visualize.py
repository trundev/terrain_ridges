"""Visualize various topology graphs
"""
import os
import argparse
...
POLYGON_HOLES = 2       # 0: drop, 1: separate polygons, others: single self-intersecting


def figure_show(lla_grid: npt.NDArray, figarg_gen: Callable) -> None:
    """Create and show plotly figure"""
    # Create plotly figure
    fig = go.Figure()

    # Add scatters as requested by 'figarg_gen'
...
def figarg_create_demgrid(lla_grid: npt.NDArray, reduce_points: int|None=REDUCE_DEM_POINTS
...
def figarg_create_lines(lla_grid_list: Sequence[npt.NDArray]) -> dict[str, npt.NDArray]:
...
def figarg_create_graph_lines(lla_grid: npt.NDArray, mgrid: npt.NDArray[np.int_], *,
        len_scale: float|None=.8, name: str|None=None) -> dict[str, Any]:
    ...
    kwargs: dict[str, Any] = figarg_create_lines((bases_arr, lines_arr))
    if name is not None:
        kwargs['name'] = f'{name} ({np.count_nonzero(mask)})'
    return kwargs

def figarg_create_mask_polygon(lla_grid: npt.NDArray, mask: npt.NDArray[np.bool], *,
    ...
    filled: Sequence[npt.NDArray] = cont_gen.filled(cut_level, mask.max())
    ...
    for poly, sub_idx in zip(filled[0], filled[1]):
        ...
        if POLYGON_HOLES == 0:
            ...
            # Add intermediate points to fully close the polygon
...
def figarg_create_mask_line(lla_grid: npt.NDArray, mask: npt.NDArray[np.bool], *,
        quad_as_tri: bool=False, cut_level: float=.5) -> dict[str, npt.NDArray]:
    """Create lon/lat kwargs for contour line around mask"""
    cont_gen = contourpy.contour_generator(x=lla_grid[..., 0], y=lla_grid[..., 1],
            z=mask, quad_as_tri=quad_as_tri)
    lines: Sequence[npt.NDArray] = cont_gen.lines(cut_level)
...
class Slider():
    ...
    def add_slider_pos(self, label: str) -> None:
    ...
    def update_layout(self) -> None:
        ...
        for idx in range(num_steps):
            ...
            steps.append(dict(method='restyle', args=[{'visible': visible[idx]}], label=self.labels[idx]))
...
def main(args: argparse.Namespace) -> None:
    ...
    dem_band: Any = gdal_utils.dem_open(args.dem_file)
    dem_band.load()
    ...
    def figarg_gen(fig: go.Figure, lla_grid: npt.NDArray) -> Iterable[dict[str, Any]]:

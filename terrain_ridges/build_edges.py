"""Build list of all edges in a DEM grid
...
type T_IndexArray = npt.NDArray[np.int_]    # topo_graph.T_IndexArray
...
def get_rel_neighbors(ndim: int, diag_lvl: int=1):
    ...
    - Order of returned relative neighbor indices (4 for 3D):
      | node  | node      | node       |
      |-------|-----------|------------|
      | _dup_ | 3: (0,+1) | 2: (+1,-1) |
      | _dup_ | **src**   | 1: (+1,0)  |
...
def prepare_edges(alt_grid: npt.NDArray, *, distance: Callable|None=None, batch_size: int
                  ) -> Iterator[tuple[T_IndexArray, npt.NDArray]]:
...
def build_edge_list(alt_grid: npt.NDArray, *, distance: Callable|None=None
    ...
    Returns
    -------
    edge_list : (node-xy, src-tgt-node, edge-index) ndarray of ints

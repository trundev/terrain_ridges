...
type T_Graph = T_IndexArray

#
# Generic tree-graph manipulations
#
def assert_graph_shape(tgt_nodes: T_Graph):
...
def reshape_graph(tgt_nodes: T_Graph, shape: Sequence[int], *, strict_unravel: bool=False) -> T_Graph:
    ...
    new_nodes = np.unravel_index(np.ravel_multi_index(tuple(tgt_nodes), src_shape), shape=shape)
    # Deserialize into 'shape'
    result[:, *np.unravel_index(serialize, shape=shape)] = new_nodes
    return result

def mask_graph(tgt_nodes: T_Graph, mask: T_MaskArray|bool) -> T_Graph:
...
def merge_graphtrees(tgt_nodes: T_Graph, edge_list: T_IndexArray, *, keep_args: bool=True
                     ) -> tuple[T_Graph, T_MaskArray]:
...
def isolate_graph_loops(tgt_nodes: T_Graph) -> T_MaskArray:
...
def expand_along_graph(vals: npt.NDArray, tgt_nodes: T_Graph) -> npt.NDArray:
...
def isolate_graphtrees(tgt_nodes: T_Graph, loop_mask: T_MaskArray|None=None
                       ) -> tuple[T_IndexArray, T_MaskArray]:
...
def cut_graph_loops(tgt_nodes: T_Graph, tree_idx: T_IndexArray, seed_mask: T_MaskArray, *,
                    sort_keys: npt.NDArray|None=None, return_copy: bool=True) -> T_Graph:
...
#
def edge_list_to_parents(edge_list: T_IndexArray) -> T_IndexArray:
...
def edge_list_to_graph(edge_list: T_IndexArray, edge_src_mask: T_MaskArray) -> T_Graph:
...
def find_main_edge_layer(edge_list: T_IndexArray) -> T_IndexArray:
    """Obtain layer where each edge was consumed"""
    assert edge_list.ndim > 2 and edge_list.shape[1] == 2, 'The "edge_list" must be multi layered'
    return np.argmax(edge_list[:, 0] == edge_list[:, 1], axis=0) - 1

def filter_edge_src_mask(edge_src_mask: T_MaskArray, edge_list: T_IndexArray) -> T_MaskArray:
...
def select_edges(edge_list: T_IndexArray, *, node_mask: T_MaskArray=np.asarray(True)
                 ) -> T_MaskArray:
...
def build_graph_layers(alt_grid: npt.NDArray, *, distance: Callable|None=None, max_layers: int=5
    ...
    if True:    # Check if highest point(s) are NOT at source-side of edge (skip topmost)
        ceil_nodes = np.nonzero(np.nanmax(alt_grid) == alt_grid.flat)[0]
        ceil_nodes = np.nonzero((main_edge_list[0, ..., np.newaxis] == ceil_nodes).any(-1))[0]
        np.testing.assert_equal(ceil_nodes.max(initial=-1) + 1, ceil_nodes.size,
                'Highest point(s) was found at source-side of an edge:'
                f' {ceil_nodes[ceil_nodes >= ceil_nodes.size]}')
        del ceil_nodes
    ...
    #
    main_edge_src_mask: T_MaskArray = np.zeros_like(main_edge_list, dtype=bool)
    ...
    for layer in range(max_layers):
        ...
        mask = np.any(main_edge_src_mask, axis=0)
        mask[~mask] = edge_mask
...
#
def num_node_children(tgt_nodes: T_Graph) -> T_IndexArray:
...
def average_seed_pos(node_pos: npt.NDArray, tree_idx: T_IndexArray, seed_mask: T_MaskArray
                     ) -> npt.NDArray:
...
def get_node_center(node_pos: npt.NDArray, tree_idx_seed: Iterable[tuple[T_IndexArray, T_MaskArray]]
                    ) -> npt.NDArray:
...
def store_topo_graph(fname: str, graph_list: list[T_Graph],
                     edge_list: T_IndexArray, edge_src_mask: T_MaskArray,
                     lla_grid: npt.NDArray) -> None:
...
if __name__ == '__main__':
    ...
    dem_name = sys.argv[1]
    dem_band: Any = gdal_utils.dem_open(dem_name)
    dem_band.load()
    ...
    if len(sys.argv) > 2:
        ...
        edges_info, graph_list = build_graph_layers(dem_band.dem_buf, distance=dist)
        if True:
            lla_grid = dem_band.xy2lonlatalt(np.moveaxis(np.indices(dem_band.shape), 0, -1))
            store_topo_graph('topo_graph.pickle', graph_list, edges_info[2], edges_info[3], lla_grid)
            del lla_grid
    ...
    import visualize
    def figarg_gen(fig: Any, lla_grid: npt.NDArray) -> Iterable[dict[str, Any]]:

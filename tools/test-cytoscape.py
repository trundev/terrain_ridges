...
def visualize_graph_hierarchy(graph_list: list[np.array], edge_list: np.array, edge_is_base: np.array, *,
        node_pos: np.array or None=None) -> None:
    """Visualize topo-graph chain using dash-cytoscape"""
    # Check argument consistency
    assert edge_is_base.shape == edge_list.shape[1:], 'Mismatch between edge_list and edge_is_base'
    ...
    for layer, par_nodes in enumerate(graph_list):
        ...
        # Edges are ravelled
        e_list = np.asarray(np.unravel_index(edge_list[layer], shape=par_nodes.shape[1:]))
        elements += build_cytoscape_elements(par_nodes, e_list, edge_is_base, edge_layer - (base_layer + layer),
...
def build_cytoscape_elements(par_nodes: np.array, edge_list: np.array, edge_is_base: np.array, edge_layer: np.array, *,
        layer:int, base_layer: int, top_layer: bool, node_pos: np.array or None=None) -> list[dict]:
    ...
    ghost_mask = ~edge_is_base.any(0) if top_layer else np.zeros_like(edge_is_base[0])
    ...
    for idx in edge_idx:
        ...
        if edge_is_base[0, idx] or ghost_mask[idx]:
            elements.append({'data': edge_data, 'classes': classes})
        # Backward edge
        if edge_is_base[1, idx]:
...
def create_dash(title:str, subtitle:str, elements: np.array) -> None:

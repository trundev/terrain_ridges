...
for /F %%x in ('python -c "from importlib import util; osgeo=util.find_spec('osgeo_utils'); print(osgeo.submodule_search_locations[0])"') do (
  if exist %%x\apps\ogr2ogr.exe (
    set OGR2OGR=%%x\apps\ogr2ogr.exe
  )
)
echo OGR2OGR set to %OGR2OGR%
%OGR2OGR% --version
if errorlevel 1 echo Error: Unable to locate GDAL ogr2ogr && exit /b 1
...
for %%x in (%ZOOM_TOL_LEVELS%) do for /f "tokens=1,2" %%g in (%%x) do (
    ...
    %OGR2OGR% -f KML "%1.tmp\%%g.kml" "%1" -simplify %%h

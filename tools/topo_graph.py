...
# Type aliases
T_MaskArray = npt.NDArray[np.bool]
T_IndexArray = npt.NDArray[np.int_]
T_Graph = T_IndexArray
...
def build_graph_layers(alt_grid: npt.NDArray, *, distance: Callable|None=None, max_layers: int=5
    ...
    if True:    # Check if highest point(s) are NOT at source-side of edge (skip topmost)
        highest_nodes = np.nonzero(np.nanmax(alt_grid) == alt_grid.flat)[0]
        np.testing.assert_equal(main_edge_list[0, highest_nodes.size:, np.newaxis]
                                == highest_nodes, False,
...
def store_topo_graph(fname: str, graph_list: list[T_Graph],
                     edge_list: T_IndexArray, edge_src_mask: T_MaskArray) -> None:
